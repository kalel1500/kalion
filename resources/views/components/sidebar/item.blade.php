...
@php
    ...
    $linkClasses = 'group flex w-full items-center '.$centerClass.' rounded-lg p-2 text-base font-medium text-heading transition duration-75 hover:bg-neutral-tertiary-medium md:sc:p-1 md:sc:flex-col md:sc:text-xs md:sc:font-normal ';
...
<li data-level="{{ $level }}" @class(['sc:hidden' => $isDropdown && $isDeepLevel])>
    ...
    @else
        <a href="{{ $href }}" @class([$linkClasses, 'bg-neutral-tertiary-medium' => $active, 'pl-11' => $isSubitem])>
            ...
            @else
                {!! $iconHtml !!}
                <span @class([$spanClasses, 'sc:text-center sc:w-full', 'flex-1 whitespace-nowrap' => isset($counter), 'text-center' => (!isset($counter) && $level === '0')])>{{ $slot }}</span>
                @isset($counter)
