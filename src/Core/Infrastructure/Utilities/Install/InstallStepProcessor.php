...
namespace Thehouseofel\Kalion\Core\Infrastructure\Utilities\Install;
...
use Thehouseofel\Kalion\Core\Infrastructure\Utilities\Install\Objects\StepDto;
...
class InstallStepProcessor
{
    ...
    {
        if (version_compare(app()->version(), '12.32.0', '<')) {
            $message = 'Para ejecutar el comando debes tener Laravel 12.32.0 o superior.';
            if ($this->command) {
    ...
    {
        $method = $this->data->rollback ? 'down' : 'up';
        ...
        foreach ($this->cached as $item) {
            try {
                ...
                $class = new $item['className'](new StepDto(
                    ...
                    down_from         : $item['down_from'],
                    examples_from     : $item['examples_from'],
                ));
                $class->prepare();
                $class->{$method}();
    ...
    protected function saveFileData(string $file, string $method): void
    {
        try {
            ...
            $from          = $this->getFrom($stepAttribute);
            $to            = $this->getTo($stepAttribute);
            $up_from       = $this->getFrom($stepAttribute, 'up');
            $down_from     = $this->getFrom($stepAttribute, 'down');
            $examples_from = $this->getFrom($stepAttribute, 'examples');
    ...
    protected function getFrom(Step $stepAttribute, ?string $force = null): string|array
    {
        $forceUp       = $force === 'up';
        $forceDown     = $force === 'down';
        $forceExamples = $force === 'examples';
